    ...
    for reachType in reachTypes:
        ...
        for row in c:
            try:
                ...
                mysqlc.execute("DELETE FROM Reachability WHERE country = 'ZZ';") # 2022-02-16 - ZZ should be excluded from stats
...
def generateHostData(c, mysqlc, date, tld, feature, screen):
    ...
    for row in c:
        try:
            (country, hosts, hosts4, hosts6, dualstack, noip) = row
            if country == "n/a":
                country_code = "ZZ"
                continue # 2022-02-16 - ZZ should be excluded from stats
                # We don't count noip for WWW or NTP because they were generated by prefixes
                # Easier to run the same query and just reset the noip hosts to 0
                if feature == "NTP" or feature == "WWW":
                    hosts = hosts - noip
                    noip  = 0
            else:
                country_code = country
            mysqlc.execute("INSERT INTO HostData (date, tld, type, country, hosts, hosts4, hosts6, dualstack, noip) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", (date,tld,feature, country_code, hosts, hosts4, hosts6, dualstack, noip))
        except Exception as e:
...
def processPing(c, tld, onFeatures, outputDir, mysqlc, date, screen):
    ...
    c.execute("DELETE FROM tmp_ping WHERE country_code = 'ZZ'") # 2022-02-16 - ZZ should be excluded from stats
...
def processPath(c, tld, onFeatures, outputDir, mysqlc, date, screen):
    ...
    c.execute("DELETE FROM tmp_path WHERE country_code = 'ZZ'") # 2022-02-16 - ZZ should be excluded from stats
