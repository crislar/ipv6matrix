...
data.filterQuery = function (query, t, req, minSampleCol) {
  if (req.params.country)
    query.where(t.country.equals(sqlEscape(req.params.country)));
  ...
  if (req.params.group) {
    ...
    query
      .select(t[sqlEscape(req.params.group)])
      .group(t.date, t[sqlEscape(req.params.group)]);
  } else {
    query.group(t.date);
  }
  ...
  if (req.query["min-sample"] && minSampleCol)
    query.having(minSampleCol + " >= " + Number(req.query["min-sample"]));
...
data.v6hosts = function (req, res) {
  ...
  var query = t.select(
    t.date,
    t.hosts.sum().as("hosts"),
    "SUM(hosts6) + SUM(dualstack) as hosts6"
  );
  query = data.filterQuery(query, t, req, "hosts").toQuery();
  console.log("v6hosts => ", query);
...
data.v6domains = function (req, res) {
  ...
  var query = t.select(
    t.date,
    t.domains.sum().as("domains"),
    t.domains6.sum().as("domains6"),
    t.www.sum().as("www"),
    t.ns.sum().as("ns"),
    t.mx.sum().as("mx"),
    t.ntp.sum().as("ntp")
...
data.reach = function (req, res) {
  ...
  var query = t.select(
    t.date,
    t.hosts4.sum().as("hosts4"),
    t.hosts6.sum().as("hosts6"),
    t.reach4.sum().as("reach4"),
    t.reach6.sum().as("reach6")
  );
...
data.ping = function (req, res) {
  ...
  var query = t.select(
    t.date,
    t.pinghosts.sum().as("hosts"),
    t.faster.sum().as("faster"),
    t.ping4.sum().as("ping4"),
    t.ping6.sum().as("ping6"),
    "(SUM(ping6) / (SUM(dualstack) + SUM(hosts6))) / (SUM(ping4) / (SUM(dualstack) + SUM(hosts4))) as pingratio"
  );
  query = data.filterQuery(query, t, req, "hosts").toQuery();
  data.renderQuery(req, res, query.text, query.values, ["date"]);
};
...
data.path = function (req, res) {
  ...
  var query = t.select(
    t.date,
    t.pathhosts.sum().as("hosts"),
    t.fewer.sum().as("fewer"),
    t.hops4.sum().as("hops4"),
    t.hops6.sum().as("hops6"),
    "(SUM(hops6) / (SUM(dualstack) + SUM(hosts6))) / (SUM(hops4) / (SUM(dualstack) + SUM(hosts4))) as hopratio"
  );
  query = data.filterQuery(query, t, req, "hosts").toQuery();
  data.renderQuery(req, res, query.text, query.values, ["date"]);
};
...
data.search = function (req, res) {
  var t = db.domains;
  var query = t
    .select(t.domain.distinct())
    .where(t.domain.like(sqlEscape(req.params.search) + "%"))
    .limit(20)
    .toQuery();
...
data.country = function (req, res) {
  ...
  var query = t.select(
    ...
    t.hosts4.sum().as("hosts4"),
    "SUM(hosts6) + SUM(dualstack) as hosts6",
    t.hosts6.sum().as("hosts6only"),
    t.dualstack.sum().as("dualstack"),
    t.noip.sum().as("noip"),
    t.pinghosts.sum().as("pinghosts"),
    t.faster.sum().as("faster"),
    t.ping4.sum().as("ping4"),
    t.ping6.sum().as("ping6"),
    t.pathhosts.sum().as("pathhosts"),
    t.fewer.sum().as("fewer"),
    t.hops4.sum().as("hops4"),
    t.hops6.sum().as("hops6")
  );
...
data.renderQuery = (req, res, query, vars, nest) => {
  ...
  // Check the query after the update of the pingratio and the hopratio
  console.log(query);
